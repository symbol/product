...
import { getContactsFromStorage, trunc } from 'utils/client';

const ValueAccount = ({ address, size, raw, position, className, onClick }) => {
	const [name, setName] = useState();
	let containerStyle = '';
	const textStyle = size === 'md' ? styles.textMd : '';
	const displayedText = (!raw && name)
		? `${name} (${trunc(address, 'address')})`
		: address;
	...
	return (
		<div className={`${styles.valueAccount} ${containerStyle} ${className}`}>
			...
			<div className={styles.addressContainer}>
				<Link className={textStyle} href={`/accounts/${address}`} onClick={handleClick}>{displayedText}</Link>
