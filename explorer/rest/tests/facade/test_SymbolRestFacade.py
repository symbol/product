...
class TestSymbolRestFacade(TestCase):
	...
	def test_reports_healthy_core_when_database_and_node_config_are_available(self):
		node_config = Mock()
		node_config.to_dict.return_value = {'baseUrl': 'http://localhost:3000'}

		with patch('rest.facade.SymbolRestFacade.SymbolDatabase') as database_factory:
			database = database_factory.return_value
			database.check_connection.return_value = True
			facade = SymbolRestFacade(db_config=Mock(), node_config=node_config)
	...
	def test_reports_database_error(self):
		...
		with patch('rest.facade.SymbolRestFacade.SymbolDatabase') as database_factory:
			database = database_factory.return_value
			database.check_connection.side_effect = OperationalError('database unavailable')
			facade = SymbolRestFacade(db_config=Mock(), node_config=node_config)
	...
	def test_reports_configuration_error(self):
		...
		with patch('rest.facade.SymbolRestFacade.SymbolDatabase') as database_factory:
			database_factory.return_value.check_connection.return_value = True
			facade = SymbolRestFacade(db_config=Mock(), node_config=node_config, config_error=ValueError('bad config'))
	...
	def test_validates_node_request_target(self):
		node_config = Mock(spec=['assert_request_allowed'])
		node_config.assert_request_allowed.return_value = 'http://localhost:3000'
