...
import { absoluteToRelativeAmount, addressFromRaw, makeRequest, networkTypeToIdentifier, networkTimestampToUnix } from '@/app/utils';
...
export class HarvestingService {
	...
	static async fetchHarvestedBlocks(networkProperties, address, searchCriteria) {
		...
		const endpoint = `${networkProperties.nodeUrl}/statements/transaction`
			+ `?receiptType=${receiptType}&targetAddress=${address}&pageSize=${pageSize}&pageNumber=${pageNumber}&order=${order}`;
