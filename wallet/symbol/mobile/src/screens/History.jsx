...
import { useDataManager, useInit, useLoading, useWalletController } from '@/app/hooks';
import { $t } from '@/app/localization';
import { colors, spacings } from '@/app/styles';
import { handleError, removeAllowedTransactions, removeBlockedTransactions } from '@/app/utils';
...
export const History = () => {
	...
	const [confirmed, setConfirmed] = useState(currentAccountLatestTransactions);
	...
	const fetchTransactions = async pageNumber => {
		...
		if (pageNumber === 1) {
			...
			filteredUnconfirmed = filter.blocked
				? removeAllowedTransactions(unconfirmed, blackList)
				: removeBlockedTransactions(unconfirmed, blackList);
			filteredPartial = filter.blocked
				? removeAllowedTransactions(partial, blackList)
				: removeBlockedTransactions(partial, blackList);
		}
		else {
		...
		const filteredConfirmed = filter.blocked
			? removeAllowedTransactions(confirmed, blackList)
			: removeBlockedTransactions(confirmed, blackList);
		...
		const isLastPage = confirmed.length < pageSize;
	...
	useInit(refresh, isWalletReady, [currentAccount?.publicKey, blackList, filter]);
	...
	// Listener
	useEffect(() => {
		WalletController.on(constants.ControllerEventName.NEW_TRANSACTION_UNCONFIRMED, refresh);
		WalletController.on(constants.ControllerEventName.NEW_TRANSACTION_PARTIAL, refresh);
		WalletController.on(constants.ControllerEventName.NEW_TRANSACTION_CONFIRMED, refresh);

		return () => {
			WalletController.removeListener(constants.ControllerEventName.NEW_TRANSACTION_UNCONFIRMED, refresh);
			WalletController.removeListener(constants.ControllerEventName.NEW_TRANSACTION_CONFIRMED, refresh);
			WalletController.removeListener(constants.ControllerEventName.NEW_TRANSACTION_PARTIAL, refresh);
		};
	}, []);
