...
import { useDataManager, useInit, useWalletController } from '@/app/hooks';
...
export const TransactionDetails = props => {
	...
	const { chainName, isWalletReady, accounts, currentAccount, networkIdentifier, networkProperties } = WalletController;
	...
	const [fetchPartialInfo, isPartialInfoLoading, partialInfo] = useDataManager(() => {
		if (transaction.type === TransactionType.AGGREGATE_BONDED) {
			...
			return api.transaction.fetchTransactionInfo(transaction.hash, transactionOptions);
		}
	});
	...
	const [fetchStatus, isStatusLoading, status] = useDataManager(() => {
		return api.transaction.fetchStatus(transaction.hash || transaction.id, networkProperties);
	}, null);
	...
	if (transaction.type === TransactionType.TRANSFER && isOutgoingTransaction(transaction, currentAccount)) {
		...
		transaction.mosaics.forEach(mosaic => {
			if (isMosaicRevokable(mosaic, networkProperties.chainHeight, currentAccount.address)) 
	...
	const openBlockExplorer = () => Linking.openURL(createExplorerTransactionUrl(
		chainName, 
		networkIdentifier, 
		transaction.hash
	));
					...
					<View>
						<FormItem clear="horizontal" style={styles.formItem}>
							...
							<StyledText style={styleAmount}>
								{transaction.amount || 0} {config.chains.symbol.ticker}
							</StyledText>
						</FormItem>
					</View>
