...
import { createTokenExplorerUrl, isMosaicRevokable } from '@/app/utils';
...
export const AssetDetails = props => {
	...
	const { chainName, currentAccount, chainHeight, networkIdentifier } = WalletController;
	...
	const handleOpenBlockExplorer = () => {
		let url;
		if (group === 'mosaic')
			url = createTokenExplorerUrl(chainName, networkIdentifier, asset.id);
		if (group === 'namespace')
			url = createNamespaceExplorerUrl(chainName, networkIdentifier, asset.id);
