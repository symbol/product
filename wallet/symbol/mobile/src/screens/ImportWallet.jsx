...
import { createOptInPrivateKeyFromMnemonic, publicAccountFromPrivateKey } from 'wallet-common-symbol';
...
export const ImportWallet = () => {
	...
	const [checkOptInAccounts] = useDataManager(
		async () => {
			...
			const optInPublicKey = publicAccountFromPrivateKey(optInPrivateKey, NetworkIdentifier.MAIN_NET).publicKey;
			
			setLoadingStep(4);
			setTimeout(() => {
				if (optInWhiteList.includes(optInPublicKey))
					saveMnemonic(optInPrivateKey)
				else
					saveMnemonic(null);
			}, 500);
		},
