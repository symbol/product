...
import { useDataManager, usePromiseMap, useValidation, useWalletController } from '@/app/hooks';
...
export const AddSeedAccount = () => {
	...
	const remainedSeedAccounts = networkSeedAccounts.filter(seedAccount => 
		!networkAccounts.some(account => account.address === seedAccount.address));
	...
	const fetchBalances = async () => {
		...
		networkSeedAccounts.forEach(seedAccount => {
			accountBalanceFetchMap[seedAccount.publicKey] = api.account.fetchAccountBalance(networkProperties, seedAccount.address);
		});
