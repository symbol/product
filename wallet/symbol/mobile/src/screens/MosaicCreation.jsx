...
import { useDataManager, useInit, usePasscode, useToggle, useValidation, useWalletController } from '@/app/hooks';
...
export const MosaicCreation = () => {
	...
	const [send] = useDataManager(
		async () => {
			await api.transaction.sendMosaicCreationTransaction(transaction, currentAccount, networkProperties);
			toggleSuccessAlert();
		},
		null,
		handleError
	...
	const fetchRentalFee = async () => {
		const rentalFees = await api.network.fetchRentalFees(networkProperties);
