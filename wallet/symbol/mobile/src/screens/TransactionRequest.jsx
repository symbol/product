...
import { useDataManager, useInit, usePasscode, useToggle, useWalletController } from '@/app/hooks';
...
export const TransactionRequest = props => {
	const { route } = props;
	...
	const [loadTransaction, isTransactionLoading] = useDataManager(
		async (payload, generationHash) => {
			...
			const transaction = await api.transaction.resolveTransactionFromPayload(
				payload,
				networkProperties,
				currentAccount,
				fillSignerPublickey
	...
	useEffect(() => {
		if (isWalletReady) 
			loadTransaction(params.data, params.generationHash);
