...
export const TransactionGraphic = props => {
	...
	const { currentAccount, networkIdentifier, networkProperties, accounts } = WalletController;
			...
			<>
				...
				{!!transferredAmount && (
					<Text style={styles.actionText}>
						{Math.abs(transferredAmount)} {config.chains.symbol.ticker}
					</Text>
				)}
			</>
		);
	...
	case TransactionType.MOSAIC_GLOBAL_RESTRICTION: {
		...
		const actionText = truncText(`${transaction.restrictionKey} ${$t(`data_${transaction.newRestrictionType}`)} `
	...
	case TransactionType.HASH_LOCK: {
		...
		ActionBody = () => (
			<Text style={styles.actionText}>
				{lockedAmount} {config.chains.symbol.ticker}
			</Text>
		);
		break;
	}
