...
const renderTypeMap = {
	...
	fee: ['maxFee', 'rentalFee', 'transactionFee', 'fee'],
	message: ['message'],
	mosaics: ['mosaics', 'tokens'],
	mosaic: ['mosaic', 'token'],
	encryption: ['isMessageEncrypted'],
...
const getTickerFromTokenName = tokenName => {
	const parts = tokenName.split('.');
	const isNamespaced = parts.length > 1;
	
	return isNamespaced ? parts[parts.length - 1].toUpperCase() : tokenName;
};
...
export const TableView = props => {
	...
	const { currentAccount, accounts, networkIdentifier } = WalletController;
					...
					ItemTemplate = (
						<View style={styles.fee}>
							...
							<StyledText type="body">
								{item.value.token.amount} {getTickerFromTokenName(item.value.token.name)}
							</StyledText>
						</View>
					);
					break;
