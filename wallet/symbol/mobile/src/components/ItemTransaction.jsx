...
export const ItemTransaction = props => {
	const { currentAccount, accounts: walletAccounts, networkIdentifier, price } = WalletController;
	...
	const dateText = !isDateHidden ? `${dateTextPrefix} ${formatDate(timestamp || deadline.timestamp, $t, true)}` : '';
	let iconSrc;
	...
	if (amount < 0) {
		amountText = `${amount} ${config.chains.symbol.ticker}`;
	...
	if (type === TransactionType.TRANSFER && isOutgoingTransaction(transaction, currentAccount)) {
		...
		const { restrictionType } = transaction;
