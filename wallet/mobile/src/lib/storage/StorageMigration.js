    ...
    static async migrate2() {
        ...
        accounts.forEach((account) => {
            ...
            if (account.type === 'hd' && account.path) {
                ...
                const walletAccount = createWalletAccount(account.privateKey, account.network, account.name, WalletAccountType.SEED, index);
        ...
        if (!networkAccounts.mainnet.length) {
            const privateKey = createPrivateKeysFromMnemonic(mnemonic, [0], NetworkIdentifier.MAIN_NET)[0];
            const walletAccount = createWalletAccount(privateKey, NetworkIdentifier.MAIN_NET, 'My Account', 'seed', 0);
            networkAccounts.mainnet.push(walletAccount);
        }
        if (!networkAccounts.testnet.length) {
            const privateKey = createPrivateKeysFromMnemonic(mnemonic, [0], NetworkIdentifier.TEST_NET)[0];
            const walletAccount = createWalletAccount(privateKey, NetworkIdentifier.TEST_NET, 'My Account', 'seed', 0);
