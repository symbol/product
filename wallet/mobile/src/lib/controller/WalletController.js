...
export class WalletController {
     ...
     * @returns {Promise<boolean>} - true if the wallet contains at least one account
     */
     ...
     * @returns {Promise<void>} - a promise that resolves when the account is selected
     */
	...
	cosignTransaction = async (transaction, password) => {
		if (transaction.type !== TransactionType.AGGREGATE_BONDED) {
			throw new AppError(
				...
				`Failed to cosign transaction. Invalid transaction type "${transaction.type}". `
				+ `Expected type "${TransactionType.AGGREGATE_BONDED}"`
			);
		}
     ...
     * @param {string} publicKey - sender (or recipient) public key
     * @param {string} [password] - wallet password
     ...
     * @returns {Promise<void>} - a promise that resolves when the network properties are fetched
     */
	fetchNetworkProperties = async nodeUrl => {
		...
		if (networkProperties.networkIdentifier !== this.networkIdentifier) {
			throw new AppError(
				...
				+ `Expected "${this.networkIdentifier}", got "${networkProperties.networkIdentifier}"`
			);
		}
