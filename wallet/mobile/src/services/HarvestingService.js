...
import { addressFromRaw, getMosaicAbsoluteAmount, getMosaicRelativeAmount, makeRequest, networkIdentifierToNetworkType, publicAccountFromPrivateKey, timestampToLocalDate } from 'src/utils';
import { Account, AccountKeyLinkTransaction, AggregateTransaction, Deadline, LinkAction, NodeKeyLinkTransaction, PersistentDelegationRequestTransaction, TransactionHttp, UInt64, VrfKeyLinkTransaction } from 'symbol-sdk';
...
export class HarvestingService {
    ...
    static async fetchHarvestedBlocks(networkProperties, address, { pageNumber = 1, pageSize = 100 }) {
        ...
        return response.data.map(el => ({
            ...
            amount: getMosaicRelativeAmount(
                el.statement.receipts.find(receipt => 
                    receipt.type === receiptType && addressFromRaw(receipt.targetAddress) === address
                )?.amount || 0,
                divisibility
            )
        }))
    ...
    static async start(networkProperties, account, nodeUrl, linkedKeys, fee) {
        ...
        if (linkedKeys.vrfPublicKey) {
            transactions.push(VrfKeyLinkTransaction.create(
        ...
        if (linkedKeys.linkedPublicKey) {
            transactions.push(AccountKeyLinkTransaction.create(
                Deadline.create(networkProperties.epochAdjustment),
        ...
        if (linkedKeys.nodePublicKey) {
            transactions.push(NodeKeyLinkTransaction.create(
                Deadline.create(networkProperties.epochAdjustment),
        ...
        transactions.push(PersistentDelegationRequestTransaction.createPersistentDelegationRequestTransaction(
            Deadline.create(networkProperties.epochAdjustment),
