...
describe('HarvestingModule', () => {
	...
	describe('fetchStatus', () => {
		it('fetches harvesting status', async () => {
			...
			expect(HarvestingService.fetchStatus).toHaveBeenCalledWith(mockRoot.networkProperties, mockRoot.currentAccount);
		});
	});
	...
	describe('createStopHarvestingTransaction', () => {
		...
		it('throws an error if no keys linked', () => {
			...
			// Act & Assert:
			expect(() => harvestingModule.createStopHarvestingTransaction()).toThrow(expectedError);
		});
	});
});
