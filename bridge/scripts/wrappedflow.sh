...
python3 -m workflows.download_wrap_requests --config "${config}" --unwrap
python3 -m workflows.send_payouts --config "${config}"
python3 -m workflows.send_payouts --config "${config}" --unwrap
python3 -m workflows.check_finalized_transactions --config "${config}"
python3 -m workflows.check_finalized_transactions --config "${config}" --unwrap
