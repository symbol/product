...
def _process_transfer_transaction(transaction_identifier, transaction_json):
	amount = 0
...
def extract_wrap_request_from_transaction(network, transaction_with_meta_json):  # pylint: disable=invalid-name
	...
	if TransactionType.MULTISIG.value == transaction_type:
		...
		if TransactionType.TRANSFER.value == inner_transaction_type:
			return _process_transfer_transaction(transaction_identifier, transaction_json['otherTrans'])
	...
	if TransactionType.TRANSFER.value == transaction_type:
		return _process_transfer_transaction(transaction_identifier, transaction_json)
