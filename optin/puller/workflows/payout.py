		...
		for address, request_group in sent_requests.items():
			...
			for request in request_group.requests:
				if request.optin_transaction_hash in finalized_transaction_hashes:
					print(f'marking request with transaction hash {request.transaction_hash} COMPLETED')
					if not self.is_dry_run:
						self.databases.inprogress.set_request_status(request, OptinRequestStatus.COMPLETED, request.transaction_hash)
