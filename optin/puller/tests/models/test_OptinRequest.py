...
class OptinRequestTest(unittest.TestCase):
	def test_can_create_request_regular(self):
		# Act:
		request = OptinRequest(Address(NEM_ADDRESS), HEIGHT, Hash256(HASH), {'type': 100, 'destination': PUBLIC_KEY_1})

		# Assert:
		self.assertEqual(Address(NEM_ADDRESS), request.address)
		self.assertEqual(HEIGHT, request.transaction_height)
	...
	def test_can_create_request_multisig(self):
		# Act:
		request = OptinRequest(Address(NEM_ADDRESS), HEIGHT, Hash256(HASH), {
	...
	def test_can_create_request_error(self):
		# Act:
		error = OptinRequestError(Address(NEM_ADDRESS), HEIGHT, Hash256(HASH), 'this is an error message')

		# Assert:
		self.assertEqual(Address(NEM_ADDRESS), error.address)
		self.assertEqual(HEIGHT, error.transaction_height)
