...
export const addressTemplate = (rowData, key, config, fixResponsiveText = false) => {
	...
	if (1 < addresses.length)
		addresses.push('');
	return <Address values={addresses} linkBaseUrl={config.keyRedirects[key] || ''} fixedLength={fixResponsiveText}/>;
};
...
export const dateTransactionHashTemplate = (rowData, key, timestampKey, config, fixResponsiveText=false) => {
	...
	return <DateTransacationHash values={hashes} timestamps={timestamps} 
		linkBaseUrl={config.keyRedirects[key] || ''} fixedLength={fixResponsiveText}/>;
};
